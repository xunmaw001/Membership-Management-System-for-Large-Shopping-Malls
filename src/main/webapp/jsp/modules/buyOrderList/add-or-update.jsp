    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑购买订单详情</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">购买订单详情管理</li>
                        <li class="breadcrumb-item active">编辑购买订单详情</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">购买订单详情信息</h3>
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 级联表的字段 -->
                                    <div class="form-group col-md-6">
                                        <label>购买订单</label>
                                        <div>
                                            <select id="buyOrderSelect" name="buyOrderSelect"
                            ...
                            <!-- 当前表的字段 -->
                                    ...
                                    <div class="form-group col-md-6">
                                        <label>订单id</label>
                                        <input id="buyOrderId" name="buyOrderId" class="form-control"
                                               placeholder="订单id">
                                    </div>
...
<script>
    ...
    function submit() {
        if (validform() == true && compare() == true) {
           ...
           if($("#buyOrderId") !=null){
               ...
               if(buyOrderId == null || buyOrderId =='' || buyOrderId == 'null'){
                   alert("订单id不能为空");
                   return;
               }
           }
           if($("#goodsId") !=null){
            ...
            httpJson("buyOrderList/" + urlParam, "POST", data, (res) => {
                if(res.code == 0)
                {
                    window.sessionStorage.removeItem('addbuyOrderList');
    ...
    // 查询列表
        ...
        function buyOrderSelect() {
            //填充下拉框选项
            http("buyOrder/page?page=1&limit=100&sort=&order=", "GET", {}, (res) => {
                if(res.code == 0){
                    buyOrderOptions = res.data.list;
                }
            });
        }

        function buyOrderSelectOne(id) {
    ...
    <!-- 初始化级联表的下拉框(要根据内容修改) -->
    <!-- 初始化级联表的下拉框(要根据内容修改) -->
        function initializationbuyOrderSelect() {
            var buyOrderSelect = document.getElementById('buyOrderSelect');
            if(buyOrderSelect != null && buyOrderOptions != null  && buyOrderOptions.length > 0 ) {
                for (var i = 0; i < buyOrderOptions.length; i++) {
                        buyOrderSelect.add(new Option(buyOrderOptions[i].name, buyOrderOptions[i].id));
                }

                $("#buyOrderSelect").change(function(e) {
                        buyOrderSelectOne(e.target.value);
                });
            }
    ...
    function setSelectOption() {
        ...
        <!--  级联表的下拉框回显  -->
            ...
            if(buyOrderSelect != null && buyOrderOptions != null  && buyOrderOptions.length > 0 ) {
                for (var i = 0; i < buyOrderOptions.length; i++) {
                    if (buyOrderOptions[i].id == ruleForm.buyOrderId) {//下拉框value对比,如果一致就赋值汉字
                        buyOrderSelect.options[i+1].selected = true;
                        $("#buyOrderSelect" ).selectpicker('refresh');
                    }
                }
            }
            var goodsSelect = document.getElementById("goodsSelect");
    ...
    // 获取当前详情
    function getDetails() {
        var addbuyOrderList = window.sessionStorage.getItem("addbuyOrderList");
        if (addbuyOrderList != null && addbuyOrderList != "" && addbuyOrderList != "null") {
            window.sessionStorage.removeItem('addbuyOrderList');
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("buyOrderList/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#buyOrderId").val(ruleForm.buyOrderId);
        $("#goodsId").val(ruleForm.goodsId);
        $("#orderNumber").val(ruleForm.orderNumber);

    }
    <!--  级联表的数据回显  -->
    function buyOrderDataBind(){

                    <!-- 把id赋值给当前表的id-->
        $("#buyOrderId").val(ruleForm.id);
